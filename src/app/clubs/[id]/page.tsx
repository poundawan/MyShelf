      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4 border-b border-border pb-6">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm text-ink-soft">
            {club.city} · {club.memberships.length} membre{club.memberships.length > 1 ? "s" : ""} ·{" "}
        ...
        {user ? (
          isMember ? (
            <form action={leaveClubAction.bind(null, club.id)}>
              <Button type="submit" variant="secondary">Quitter le club</Button>
            </form>
          ) : (
          ...
          <Link href="/login" className="inline-block">
            <Button>Connexion pour rejoindre</Button>
          </Link>
        )}
      </div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 sm:grid-cols-[minmax(0,1fr)_260px]">
        <div>
          <h2 className="font-display text-lg text-cream">
            {club.events.length > 0 ? "Les tables à venir" : "Aucune table prévue"}
          </h2>
          {club.events.length === 0 ? (
            <Card className="mt-3 p-6 text-sm text-ink-soft">
              Personne n&apos;a encore ouvert de table pour ce club.
              {isMember && " À toi de lancer la prochaine ?"}
            </Card>
          ) : (
          ...
          {isMember && (
            <Link href="/events/new" className="mt-5 inline-block">
              <Button variant="secondary">Ouvrir une table</Button>
            </Link>
          )}
        </div>
        ...
        <div>
          <Card className="p-4">
            <h3 className="mb-3 text-sm font-bold text-cream">
              Membres ({club.memberships.length})
            </h3>
              ...
              {club.memberships.map((m) => (
                <Link key={m.id} href={`/profile/${m.userId}`} className="flex items-center gap-2.5 hover:opacity-80">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-ink-soft">{playerLevelLabels[m.user.experienceLevel]}</div>
                  </div>
                  {m.userId === user?.id && <Badge variant="outline">Toi</Badge>}
                </Link>
              ))}
            </div>
          </Card>
